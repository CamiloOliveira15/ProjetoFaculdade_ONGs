...
async function handleCadastroFormSubmit(event) {
    ...
    const form = event.target; // Pega o formulário do evento

    // --- CORREÇÃO: LÓGICA DE VALIDAÇÃO PÓS-SUBMIT ---
    // 1. Limpa mensagens de status antigas
    statusMessage.innerHTML = '';
    
    // 2. Adiciona a classe para ativar os estilos de validação do style.css
    form.classList.add('form-submitted');

    // 3. Verifica a validade nativa do HTML5
    if (!form.checkValidity()) {
        // 4. Se for inválido, exibe uma mensagem de erro genérica e para.
        // O CSS (style.css) cuidará de destacar os campos errados.
        statusMessage.innerHTML = `
            <div class="rounded-md bg-red-100 p-4 text-red-700 dark:bg-red-900 dark:text-red-200">
                <strong>Erro no formulário:</strong> Por favor, preencha todos os campos obrigatórios corretamente.
            </div>
        `;
        
        // Tenta focar no primeiro campo inválido para acessibilidade
        const firstInvalidField = form.querySelector(':invalid');
        if (firstInvalidField) {
            firstInvalidField.focus();
        }
        
        return; // Para a execução, não envia para a API
    }
    // --- FIM DA CORREÇÃO ---
    ...
    try {
        ...
        if (response.ok) {
            ...
            // Remove a classe de 'submitted' para limpar as bordas vermelhas
